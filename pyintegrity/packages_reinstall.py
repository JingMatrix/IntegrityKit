...
import tempfile
from . import adb
from .packages_patch import _patch_packages
...
def _reinstall_command(package_name, origin):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        _patch_packages(
            ...
            target_filter=None,  # Not needed when target_package is specified
            origin_package=origin,
            package_source='2',  # Default to 'Store' for a reinstall
            no_backup=True,    # We don't need another backup
            apply_changes=False,
            full_reboot=False
        )
